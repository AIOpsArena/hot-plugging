...
from DiagFusion.public_function import save, load
...
class FastTextLab:
    def __init__(self, config, action, cases, split=True):
        self.config = config
        self.cases = cases
        ...
        self.anomaly_types = np.append('[normal]', cases['failure_type'].unique())
        ...
        print('anomaly_type_labels', self.anomaly_type_labels)
        print('node_label', self.node_labels)
        self.train_data = self.prepare_data()

    def prepare_data(self):
        metric_trace_text_path = self.config['text_path']
        temp_data = load(metric_trace_text_path)
        train = self.cases.index
...
def run_fasttext(config, run_table, action):
    # event embedding流程；基于数据增强
    start_ts = time.time()
    lab2 = FastTextLab(config, action, run_table)
    lab2.do_lab(action)
    end_ts = time.time()
